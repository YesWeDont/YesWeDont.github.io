...
export function initiate(chdir:(newDirection:Direction) => void):Promise<Game>{
    return new Promise((res)=>document.addEventListener('DOMContentLoaded', res)).then(()=>{
        //renderer
        let ctx:CanvasRenderingContext2D,
        canvas = document.querySelector('canvas');
        if(!canvas) throw new Error("Cannot find the game renderer")
        let WonH = (innerWidth-10)/(innerHeight-10)
        canvas.height = innerHeight - 10;
        let cellWidth = Math.floor((innerHeight-10) / 15),
        xAxisCellNumber = Math.floor(WonH * 15)
        canvas.width = cellWidth * xAxisCellNumber;

        //keybinds
        document.addEventListener("keydown",(e)=>{
            if(e.key.toLowerCase() === 'w' || e.key === 'ArrowUp') chdir("n")
